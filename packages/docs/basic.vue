<template>
  <Markdown :content="testContent" :components="components" :remark-plugins="[remarkGfm]">
    <template #code="{ children, className, node, ...rest }">
      <t-tag v-if="!/language-(\w+)/.exec(node.properties?.className ?? '')" :theme="'primary'" :variant="'light'">{{
        children
      }}</t-tag>
      <code v-else v-bind="{ ...rest }" :className="className"> {{ children }} </code>
    </template>
    ...
    <template #hr>
      <t-divider align="left">这是用 TDesign 替换的 分割线</t-divider>
    </template>
  </Markdown>
</template>
